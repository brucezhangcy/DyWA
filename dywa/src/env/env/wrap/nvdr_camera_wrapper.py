...
def debug_image(cam_out,
    ...
    from util.torch_util import dcn
    from util.vis.img import digitize_image, normalize_image
    import cv2
        ...
        for e in self.env.envs:
            ...
            for i in range(num_actors):
                ...
                if 'filename' in asset_args:
                    # URDF
                    # Originally the heuristic was: if 'robots' is in the
                    # filename, this is the robot URDF (lives in project
                    # asset_root); else this is an object URDF (was
                    # materialized to scene tmpdir = new_root).
                    # This breaks for Trossen, whose URDF path is
                    # 'trossen_arm/urdf/wxai_base.urdf' (no 'robots'
                    # substring) → tries to load from tmpdir → FileNotFound.
                    # More robust: try new_root first, fall back to original
                    # rootpath if the file isn't actually there.
                    asset_file = asset_args['filename']
                    if new_root is not None and \
                       os.path.isfile(os.path.join(new_root, asset_file)):
                        asset_root = new_root
                    else:
