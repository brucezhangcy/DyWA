...
class NvdrRecordEpisode(WrapperEnv):
    ...
    def __record(self, obs, rew, done, info):
        ...
        buf_len = self.__images.shape[0]
        overflow = self.__index >= buf_len
        if overflow.any():
            self.__index[overflow] = 0
        ...
        # DEBUG: temporary diagnostic for trossen export failure
        if done_np.any():
            steps = dcn(self.env.buffers['step'])
            n_done = int(done_np.sum())
            done_envs = np.argwhere(done_np).ravel().tolist()
            done_steps = [int(steps[i]) for i in done_envs]
            succ_flag = [bool(info['success'][i]) if 'success' in info else None
                         for i in done_envs]
            print(f'[NvdrRec] done event: n_done={n_done} envs={done_envs} '
                  f'steps={done_steps} success={succ_flag}', flush=True)

        for env_id in np.argwhere(done_np).ravel():
            if self.env.buffers['step'][env_id] < 30:
                print(f'[NvdrRec]   SKIP env={int(env_id)} step={int(self.env.buffers["step"][env_id])} < 30', flush=True)
                continue
            if (sel is not None) and (not sel[env_id]):
                continue
            print(f'[NvdrRec]   EXPORT env={int(env_id)} step={int(self.env.buffers["step"][env_id])} count={int(self.__index[env_id])}', flush=True)
