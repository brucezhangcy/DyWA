...
from env.common import quat_rotate
from env.util import draw_sphere
from env.task.push_with_arm_task import PushWithArmTask
from util.config import ConfigBase
from util.torch_util import dcn
from util.math_util import apply_pose, apply_pose_tq, matrix_from_quaternion
...
TrossenArmDofPos    = spaces.Box(-2 * np.pi, +2 * np.pi, (8,))   # 6 arm + 2 gripper carriage
TrossenArmDofPosVel = spaces.Box(-2 * np.pi, +2 * np.pi, (16,))  # 8 DOF × (pos, vel)
...
OBS_SPACE_MAP = {
    ...
    'pos6':     TrossenArmDofPos,
    'pos_vel6': TrossenArmDofPosVel,
    'wrench': Wrench,
    'mass': Mass,
...
def _get_obs_bound_map():
    ...
    pos6     = ([0] * 8, [1] * 8)   # Trossen has 8 DOFs (6 arm + 2 gripper carriage)
    vel6     = ([0] * 8, [1] * 8)
